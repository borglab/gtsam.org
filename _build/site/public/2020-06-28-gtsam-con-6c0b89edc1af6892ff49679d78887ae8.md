...
the world coordinate frame ($^w\tilde{X}$).

Math: $^w\tilde{X} =~^wT_c~\cdot~^c\tilde{X}$
...
to bring points from the world coordinate frame ($^w\tilde{X}$) to
the camera coordinate frame ($^c\tilde{X}$).

Math: $^c\tilde{\mathbf{X}} =~\left(^wT_c\right)^{-1}~\cdot~^w\tilde{X}$
...
\begin{align*}
^{sensor}\tilde{X}
&=~^{sensor}T_{world}~\cdot~^{world}\tilde{X}\\
&=~^{sensor}T_{body}~\cdot~^{body}T_{world}~\cdot~^{world}\tilde{X}\\
&= \left(^{world}T_{body}~\cdot~^{body}T_{sensor}\right)^{-1}~\cdot~^{world}\tilde{X}
\end{align*}
$$
...
```
It will project the world 3D point $^{world}\tilde{X}$ into the sensor coordinates like so:
```cpp
