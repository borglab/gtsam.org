...
Then from a terminal, starting in the root library folder, execute the commands below for an out-of-source build. This will build the library and unit tests (to the default system install path), run all of the unit tests, and then install the library itself.
