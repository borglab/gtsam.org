...
($^wT_c$) to bring points from the camera coordinate frame ($^c\widetilde{X}$) to
the world coordinate frame ($^w\widetilde{X}$).

Math: $^w\widetilde{X} =~^wT_c~\cdot~^c\widetilde{X}$
...
to bring points from the world coordinate frame ($^w\widetilde{X}$) to
the camera coordinate frame ($^c\widetilde{X}$).

Math: $^c\widetilde{X} =~\left(^wT_c\right)^{-1}~\cdot~^w\widetilde{X}$
...
\begin{align*}
^{sensor}\widetilde{X}
&=~^{sensor}T_{world}~\cdot~^{world}\widetilde{X}\\
&=~^{sensor}T_{body}~\cdot~^{body}T_{world}~\cdot~^{world}\widetilde{X}\\
&= \left(^{world}T_{body}~\cdot~^{body}T_{sensor}\right)^{-1}~\cdot~^{world}\widetilde{X}
\end{align*}
$$
...
```
It will project the world 3D point $^{world}\widetilde{X}$ into the sensor coordinates like so:
```cpp
