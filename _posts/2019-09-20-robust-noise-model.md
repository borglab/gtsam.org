...
Random Sample Consensus, or RANSAC for short, has been a fundamental tool for parameter estimation in many a roboticist's toolbox for years. While RANSAC has proven to work well in practice, it is not robust to outliers, necessitating the need for a substantially high number runs of RANSAC to converge to a good solution.  We advocate for better, more robust approaches to parameter estimation which are less sensitive to outliers, but are conceptually easy and intuitive like RANSAC. Moreover, these approaches are amenable to easy plug-and-play use in pre-existing optimization frameworks, requiring minimal changes to existing pipelines.

In this blog post, we demonstrate the capabilities of Robust Noise Models, models that downweight outliers to provide better estimates of the final outliers which are less influenced by these outliers. 

We begin by reviewing techniques for paramter estimation as outlined by the [tutorial by Zhengyou Zhang][1]. We then explain robust noise models, showcasing their ease of use with factor graph based optimization on a simple `SE(2)` estimation problem, instead of on generic cone fitting problems. Finally, we show that given robust noise models, we can easily combine them with RANSAC to give us very good results which supercede either approach.
...
One answer to this question is a family of models known as __Robust Noise Models__ or __M-estimators__. The M-estimators try to reduce the effect of outliers by replacing the squared residuals with a function of the residuals $\rho$ that weighs each residual term by some value:
...
Now it's time for the real deal. So far we've spoken about robust estimators from a theoretical perspective, with an application to cone fitting that doesn't really give intuition about the power of this technique. From a pedagogical perspective, it would be imperative to add a concrete example to demonstrate the power of a robust noise model, which in this specific case is the **Huber M-estimator**, though any other provided M-estimator can be used depending on the application or preference.

For our example application, we model the case of estimating an `SE(2)` transformation between two images, a scenario commonly seen in PoseSLAM applications. For our images, we use an example from the KITTI dataset, shown below. This image provides a strong motivating example due to its applicability towards autonomous vehicles.

![original image from KITTI](/assets/images/robust_estimators/kitti1.png)

To model an `SE(2)` transformation, we apply a perspective transform to the image. The transformation applied is `(x, y, theta) = (3.7, 3.502, 0.0144862)`. This gives us a ground truth value to compare our methods against. The transformed image can be seen below.

![Warped KITTI image](/assets/images/robust_estimators/kitti2.png)
...
Running the above code give us the transform values `(4.03675, 4.04491, 0.0137934)`. As you can see, while these are close, they are not a very good estimate, and this error can quickly accumulate to really throw off our estimates.

Now how about we try using M-estimators via the built-in robust noise models? This is a single line change as illustrated below:
...
// We define our robust noise model here, providing the default parameter value for the estimator.
...
for (vector<tuple<int, Point2, int, Point2>>::iterator it = matches.begin();
    ...
    // Add the Point2 expression variable, an initial estimate, and the measurement noise.
    // The graph takes in factors with the robust noise model.
...
This version of the code gives us the `SE(2)` values `(3.70856, 3.50218, 0.0144862)` which is pretty much spot on, despite the initial estimate for the transform being arbitrary. This makes it apparent that the use of robust estimators and subsequently robust noise models is the way to go for use cases where outliers are a concern.

But you may ask how does this compare to our dear old friend RANSAC? Surely RANSAC can perform as well as using the robust noise model, so why do we even need all this extra overhead? Using the OpenCV implementation of RANSAC, a similar pipeline gives use the following `SE(2)` values: `(3.93109178 3.57635549, 0.014447611237753911)`.

That's pretty close too, but not quite as correct as the result of our robust noise model. This minor error can accumulate pretty quickly, especially for long range trajectories, thus having more accurate and robust methods of state estimation are vital, especially in the domain of autonomous vehicles.


<!-- ## Robust Noise Models + RANSAC -->
